...
浏览器打开 `/tasks`：所有数据脚本可点击运行（串行，运行中拒绝新任务）、
配置定时规则（时间+星期几，错过 30 分钟内补跑）、查看历史运行记录与完整日志。
脚本日志落盘 `logs/tasks/run_{id}.log`。

定时建议：每天收盘后 16:35 跑「每日增量更新」（含缺口自愈 + 情绪重算）。
注意：应用无鉴权，局域网内任何人可触发任务（与现有页面同风险级别）。

`update` 已是增量：按交易日批量拉（一天全市场仅 1 次请求/接口），
并自动扫描已覆盖区间内的缺失交易日补洞。
